...
function state_global_set(newState, allowRedefinition) {
	...
	if (variable_struct_exists(global.gStates, newStateName) and (allowRedefinition == STATE_DEFINITION_BEHAVIOR.GlobalConstant))
		throw($"ERROR -- Tried to redefine State with name: {newStateName}");
		
	global.gStates[$ newStateName] = newState;
